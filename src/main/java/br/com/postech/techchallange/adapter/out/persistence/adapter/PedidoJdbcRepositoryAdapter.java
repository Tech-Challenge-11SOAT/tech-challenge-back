...
public class PedidoJdbcRepositoryAdapter extends GenericJdbcRepository<PedidoEntity> implements PedidoRepositoryPort {
    ...
    private static final String LISTAR_PEDIDOS_POR_CLIENTE;
    ...
    static {
        ...
        LISTAR_PEDIDOS_COMPLETOS = SQL_BASE + " ORDER BY p.data_pedido ASC";

        StringBuilder porStatus = new StringBuilder(SQL_BASE);
        porStatus.append("WHERE p.id_status_pedido = ? ORDER BY p.data_pedido ASC");
        LISTAR_PEDIDOS_POR_STATUS = porStatus.toString();

        StringBuilder porCliente = new StringBuilder(SQL_BASE);
        porCliente.append("WHERE p.id_cliente = ? ORDER BY p.data_pedido ASC");
        LISTAR_PEDIDOS_POR_CLIENTE = porCliente.toString();
    }
    ...
    public List<Pedido> listarPorCliente(Long idCliente) {
        logger.info("Iniciando método para buscar os pedido por cliente: {} - Select: {}", idCliente, LISTAR_PEDIDOS_POR_CLIENTE);
        if (idCliente == null) {

            // Buscar pedidos de clientes não identificados (id_cliente IS NULL)
            String sql = SQL_BASE + "WHERE p.id_cliente IS NULL ORDER BY p.data_pedido ASC";

            return super.jdbcTemplate.query(sql, (rs, rowNum) -> PedidoMapper.toDomain(new PedidoRowMapper().mapRow(rs, rowNum)));

        } else {
            return super.jdbcTemplate.query(LISTAR_PEDIDOS_POR_CLIENTE, (rs, rowNum) -> PedidoMapper.toDomain(new PedidoRowMapper().mapRow(rs, rowNum)), idCliente);
